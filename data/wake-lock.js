// Keeps the operator's display awake while JS8LAN or the WSPR beacon is open.
//
// The problem this solves is not cosmetic. A sleeping phone backgrounds the tab,
// the AUD1 WebSocket closes mid-session, and data.js aborts the running encoder
// on visibilitychange -- so an unattended beacon quietly stops transmitting.
//
// navigator.wakeLock is secure-context only and the firmware serves plain HTTP
// from a LAN address (there is no WiFiClientSecure or esp_https_server in the
// sketch), so on http://192.168.x.x the standard API is simply undefined. That
// is the normal case here, not the exception. The fallback is the long-standing
// NoSleep.js technique: a muted, looping video. Browsers keep the screen lit
// while video plays, and that works over plain HTTP.
//
// The video is a MOBILE technique and is only used as one. iOS ignores it
// outright, and on a desktop it was measured to hold nothing at all (2026-07-27,
// GNOME: Chrome took no idle inhibitor for our one-pixel video, for a visible
// 320x180 one, or for that one fullscreen). Everywhere it cannot help, the keeper
// says so and sends the operator to the system settings instead of burning
// battery on a video and showing a "screen on" that is not true.
//
// What this does NOT fix: the operator switching to another app. The screen stays
// lit, but the tab still goes hidden and data.js still aborts the frame in flight.
// Wake lock and app switching are different problems; only the first one is here.

(function (root, factory) {
  const value = factory();
  if (typeof module === "object" && module.exports) module.exports = value;
  else root.WakeLockKeeper = value;
})(typeof globalThis !== "undefined" ? globalThis : self, function () {

  // How the display is being held, in the order the keeper tries them.
  //   "wake-lock"   Screen Wake Lock API holds it (secure context only).
  //   "video"       The muted looping video holds it.
  //   "needs-tap"   The video was blocked by autoplay policy; one gesture fixes it.
  //   "unavailable" Nothing can hold it -- iOS over HTTP, or no video support.
  //   "idle"        Not started, or stopped.
  const STATES = ["idle", "wake-lock", "video", "needs-tap", "unavailable"];

  // Nothing about the state is written on the page. The topbar carries one dot
  // that changes colour -- green held, amber recoverable, red not held -- and
  // every word lives in the hover/tap panel. Both held states share a colour
  // because to the operator they mean the same thing; which mechanism is holding
  // the display is a developer question and lives in data-wakelock-state.
  //
  // LABELS are never displayed. They are the accessible name, because a bare
  // coloured dot tells a screen reader nothing.
  const LABELS = {
    idle: "",
    "wake-lock": "screen on",
    video: "screen on",
    "needs-tap": "tap to keep screen on",
    unavailable: "screen may sleep",
  };

  // The whole explanation, since there is nowhere else for it to go. Each one
  // opens by naming the state, because a colour on its own is ambiguous.
  //
  // No protocol talk: nobody reading this can put TLS in front of the firmware,
  // so telling them the API "needs HTTPS" only raises a question with no answer.
  // The reason it works this way belongs in the comments above and in
  // docs/wake-lock.md.
  const TIPS = {
    "wake-lock": "Screen on. The display will not sleep while this page is open.",
    video: "Screen on. The display will not sleep while this page is open — a "
      + "silent looping video keeps it lit.",
    "needs-tap": "Screen may sleep. The browser blocked what keeps the display "
      + "lit. Tap here, or anywhere on the page, to allow it.",
    unavailable: "Screen may sleep. This page cannot keep the display lit. Turn "
      + "off the automatic screen lock in the system settings before leaving the "
      + "station unattended.",
    "unavailable-ios": "Screen may sleep. This page cannot keep the display lit "
      + "on iOS. Turn off Settings → Display & Brightness → Auto-Lock before "
      + "leaving the station unattended.",
    "unavailable-desktop": "Screen may sleep. This page cannot keep the display "
      + "lit in a desktop browser. Turn off the screen blanking and sleep in the "
      + "system settings before leaving the station unattended.",
  };

  // iPadOS 13+ reports a desktop Safari user agent, so the touch points are what
  // actually give an iPad away.
  function isIos(nav) {
    if (!nav) return false;
    if (/iPad|iPhone|iPod/.test(String(nav.userAgent || ""))) return true;
    return nav.platform === "MacIntel" && Number(nav.maxTouchPoints || 0) > 1;
  }

  // The video trick is a mobile technique -- that is what NoSleep.js was written
  // for. Measured on a GNOME desktop 2026-07-27: Chrome held no idle inhibitor
  // for our one-pixel video, nor for a visible 320x180 one, nor for that one
  // fullscreen. So on a desktop the video buys nothing and claiming otherwise
  // would put a false "screen on" in front of the operator.
  function isMobile(nav) {
    return Boolean(nav) && /Android|Mobile/i.test(String(nav.userAgent || ""));
  }

  // The keeper is a factory over injected navigator/document rather than a
  // singleton reaching for globals, so that every branch can be driven from Node
  // in tools/wake-lock-smoke.js. A browser only ever shows one of them: whether
  // navigator.wakeLock exists at all is decided by the origin, and no page can
  // exercise both the secure and the insecure path in one run.
  function create(deps) {
    deps = deps || {};
    const nav = deps.navigator || null;
    const doc = deps.document || null;
    const media = deps.media || MEDIA;

    let state = "idle";
    // Why the display is not held: "ios", "desktop" or "no-video". Only the
    // wording of the advice depends on it; the state does not.
    let reason = "";
    let sentinel = null;
    let video = null;
    let started = false;
    let gestureBound = false;
    let pending = Promise.resolve();
    const listeners = [];

    function setState(next, why) {
      const nextReason = why || "";
      if (state === next && reason === nextReason) return;
      state = next;
      reason = nextReason;
      for (const listener of listeners) listener(state);
    }

    function visible() {
      return !doc || doc.visibilityState === undefined
        || doc.visibilityState === "visible";
    }

    // The sentinel is released by the browser whenever the document hides, and a
    // request while hidden rejects, so re-acquiring on visibilitychange is not an
    // optimisation -- without it the lock is gone after the first tab switch.
    async function tryWakeLock() {
      if (!nav || !nav.wakeLock || typeof nav.wakeLock.request !== "function")
        return false;
      if (!visible()) return false;
      try {
        sentinel = await nav.wakeLock.request("screen");
      } catch (_error) {
        sentinel = null;
        return false;
      }
      if (sentinel && typeof sentinel.addEventListener === "function") {
        sentinel.addEventListener("release", () => {
          sentinel = null;
          if (started && state === "wake-lock") setState("idle");
        });
      }
      setState("wake-lock");
      return true;
    }

    function buildVideo() {
      if (video || !doc || typeof doc.createElement !== "function") return video;
      const element = doc.createElement("video");
      // muted must be set before play() so the browser classifies the element as
      // inaudible: that is what keeps it from taking audio focus and from posting
      // a media notification with a pause button the operator could hit. The
      // vendored file does carry an audio track (see MEDIA below), so this is
      // load-bearing, not decorative.
      element.muted = true;
      element.defaultMuted = true;
      element.loop = true;
      element.playsInline = true;
      element.setAttribute("muted", "");
      element.setAttribute("loop", "");
      element.setAttribute("playsinline", "");
      element.setAttribute("title", "keeps the display awake");
      element.setAttribute("aria-hidden", "true");
      // Not display:none -- a hidden video is allowed to be dropped from the
      // compositor, and with it the wake lock. One transparent pixel is enough.
      element.style.cssText = "position:fixed;left:0;bottom:0;width:1px;"
        + "height:1px;opacity:0;pointer-events:none";
      for (const type of ["mp4", "webm"]) {
        if (!media[type]) continue;
        const source = doc.createElement("source");
        source.setAttribute("src", "data:video/" + type + ";base64," + media[type]);
        source.setAttribute("type", "video/" + type);
        element.appendChild(source);
      }
      if (doc.body && typeof doc.body.appendChild === "function")
        doc.body.appendChild(element);
      video = element;
      return video;
    }

    async function tryVideo() {
      // On iOS the video plays happily and the screen sleeps anyway, and on a
      // desktop it holds nothing either. In both cases playing it would cost
      // battery and, worse, tell the operator the display is safe when it is not.
      if (isIos(nav)) { setState("unavailable", "ios"); return false; }
      if (!isMobile(nav)) { setState("unavailable", "desktop"); return false; }
      const element = buildVideo();
      if (!element || typeof element.play !== "function") {
        setState("unavailable", "no-video");
        return false;
      }
      try {
        await element.play();
      } catch (_error) {
        setState("needs-tap");
        bindGesture();
        return false;
      }
      setState("video");
      return true;
    }

    // Autoplay of muted video is normally allowed, but Low Power Mode and some
    // data-saver modes reject play(). A real user gesture always passes, so the
    // next tap anywhere -- including on the dot itself -- retries.
    function bindGesture() {
      if (gestureBound || !doc || typeof doc.addEventListener !== "function") return;
      const retry = () => {
        unbindGesture();
        if (started) acquire();
      };
      gestureBound = {retry};
      for (const name of ["pointerdown", "keydown"])
        doc.addEventListener(name, retry, {once: true, capture: true});
    }

    function unbindGesture() {
      if (!gestureBound) return;
      const retry = gestureBound.retry;
      gestureBound = false;
      if (!doc || typeof doc.removeEventListener !== "function") return;
      for (const name of ["pointerdown", "keydown"])
        doc.removeEventListener(name, retry, {capture: true});
    }

    function acquire() {
      pending = pending.then(async () => {
        if (!started || !visible()) return state;
        if (sentinel) return state;
        if (video && !video.paused) { setState("video"); return state; }
        if (await tryWakeLock()) return state;
        await tryVideo();
        return state;
      });
      return pending;
    }

    function onVisibility() {
      if (!started) return;
      if (visible()) acquire();
    }

    function start() {
      if (started) return pending;
      started = true;
      if (doc && typeof doc.addEventListener === "function")
        doc.addEventListener("visibilitychange", onVisibility);
      return acquire();
    }

    function stop() {
      started = false;
      unbindGesture();
      if (doc && typeof doc.removeEventListener === "function")
        doc.removeEventListener("visibilitychange", onVisibility);
      if (sentinel && typeof sentinel.release === "function") {
        try { sentinel.release(); } catch (_error) { /* leaving anyway */ }
      }
      sentinel = null;
      if (video && typeof video.pause === "function") {
        try { video.pause(); } catch (_error) { /* leaving anyway */ }
      }
      setState("idle");
    }

    return {
      start, stop, acquire,
      get state() { return state; },
      get reason() { return reason; },
      onChange(listener) { listeners.push(listener); return listener; },
      label() { return LABELS[state] || ""; },
      tip() {
        if (state === "unavailable" && TIPS["unavailable-" + reason])
          return TIPS["unavailable-" + reason];
        return TIPS[state] || "";
      },
    };
  }

  // The dot lives in the shared site-topbar next to the firmware version, and it
  // builds its own markup and style so that adding the keeper to a page is one
  // script tag -- no edits to data.html, wspr.html, data.css and wspr.css for
  // every future tweak.
  //
  // 7 px plus a glow, matching .trx-dot in data.css; the panel matches .help-tip
  // in setup.css. Colours come from the data.css palette, which wspr.html also
  // loads, with literal fallbacks so a dot never lands invisible.
  //
  // The one place this departs from .help-tip: that rule fires on :hover alone,
  // and a phone has no hover. The panel therefore also opens on :focus-visible
  // and on .is-open, which a tap toggles -- otherwise the explanation would be
  // unreachable on exactly the devices this feature exists for.
  const DOT_CSS = ".wakelock-dot{position:relative;display:inline-block;width:7px;"
    + "height:7px;margin-left:.7em;padding:0;border:0;border-radius:50%;"
    + "vertical-align:middle;cursor:help;-webkit-tap-highlight-color:transparent;"
    + "background:var(--green,#5ad18a);box-shadow:0 0 7px var(--green,#5ad18a)}"
    + ".wakelock-dot[hidden]{display:none}"
    + ".wakelock-dot[data-wakelock-state=\"needs-tap\"]{cursor:pointer;"
    + "background:var(--amber,#ffbf69);box-shadow:0 0 7px var(--amber,#ffbf69)}"
    + ".wakelock-dot[data-wakelock-state=\"unavailable\"]{"
    + "background:var(--red,#ff6b6b);box-shadow:0 0 7px var(--red,#ff6b6b)}"
    + ".wakelock-dot::after{content:attr(data-tip);position:absolute;"
    + "bottom:calc(100% + 10px);right:-6px;width:260px;"
    + "max-width:calc(100vw - 48px);background:#101a14;"
    + "border:1px solid var(--line,#28443e);color:var(--text,#e8f3ee);"
    + "padding:10px 12px;border-radius:10px;font-size:.82rem;font-weight:400;"
    + "line-height:1.5;letter-spacing:0;text-align:left;white-space:normal;"
    + "pointer-events:none;opacity:0;transition:opacity .15s;z-index:100}"
    + ".wakelock-dot:hover::after,.wakelock-dot:focus-visible::after,"
    + ".wakelock-dot.is-open::after{opacity:1}";

  function attachDot(keeper, doc) {
    if (!doc || typeof doc.createElement !== "function") return null;
    const host = doc.querySelector(".site-topbar .tabs") || doc.querySelector(".site-topbar");
    if (!host) return null;
    if (!doc.getElementById("wakeLockDotStyle")) {
      const style = doc.createElement("style");
      style.id = "wakeLockDotStyle";
      style.textContent = DOT_CSS;
      (doc.head || host).appendChild(style);
    }
    // A button in every state, and honestly so: the dot carries no words, so
    // opening the explanation is what a tap is FOR. In needs-tap the same tap
    // additionally revives the video, but that happens through the document-level
    // gesture listener in bindGesture(), not here -- any tap on the page will do.
    const dot = doc.createElement("span");
    dot.id = "wakeLockDot";
    dot.className = "wakelock-dot";
    dot.setAttribute("role", "button");
    dot.setAttribute("tabindex", "0");
    dot.hidden = true;

    let open = false;
    function setOpen(next) {
      open = next;
      dot.classList.toggle("is-open", open);
      dot.setAttribute("aria-expanded", open ? "true" : "false");
    }
    dot.addEventListener("click", event => {
      if (event && typeof event.stopPropagation === "function") event.stopPropagation();
      setOpen(!open);
    });
    dot.addEventListener("keydown", event => {
      if (event.key === "Escape") return setOpen(false);
      if (event.key !== "Enter" && event.key !== " ") return;
      if (typeof event.preventDefault === "function") event.preventDefault();
      setOpen(!open);
    });
    // A tap-opened panel has to be dismissable by tapping away from it, or on a
    // phone it covers the topbar until the page reloads.
    if (typeof doc.addEventListener === "function") {
      doc.addEventListener("click", () => { if (open) setOpen(false); });
      doc.addEventListener("keydown", event => {
        if (open && event.key === "Escape") setOpen(false);
      });
    }
    host.appendChild(dot);

    function render() {
      const state = keeper.state;
      dot.hidden = state === "idle";
      // The tip is the only place the words live, so it doubles as the title:
      // that keeps the explanation reachable even if the injected CSS is lost.
      dot.setAttribute("data-tip", keeper.tip());
      dot.setAttribute("title", keeper.tip());
      dot.setAttribute("aria-label", keeper.label());
      dot.dataset.wakelockState = state;
      if (state === "idle") setOpen(false);
    }
    keeper.onChange(render);
    setOpen(false);
    render();
    return dot;
  }

  // Autostart, browser only. The keeper holds the display for as long as the page
  // is open rather than tracking beacon state: JS8LAN claims its session on load
  // and the WSPR page is opened to be left running, so "open" and "working" are
  // the same thing in practice, and a state machine here would only add ways to
  // get it wrong.
  function autostart(root) {
    const doc = root && root.document;
    if (!doc) return null;
    const keeper = create({navigator: root.navigator, document: doc});
    const begin = () => { attachDot(keeper, doc); keeper.start(); };
    if (doc.readyState === "loading")
      doc.addEventListener("DOMContentLoaded", begin, {once: true});
    else begin();
    root.addEventListener("pagehide", () => keeper.stop());
    return keeper;
  }

  // ---------------------------------------------------------------------------
  // Vendored media -- NoSleep.js v0.12.0, Rich Tibbett, MIT. See
  // data/THIRD-PARTY-NOTICES.txt. A 1.08 s clip that loops; it carries a silent
  // audio track besides the video track, which is why buildVideo() sets muted
  // before play().
  //
  // Cost: 12 440 B of base64 here, 5 235 B of it once gzipped (mp4 1 816 B, webm
  // 3 433 B); the whole minified and gzipped wake-lock.js.gz that ships in the
  // image is 7 982 B. If the filesystem ever runs out of room, this is the LAST
  // place to look, not the first: the alternative is canvas.captureStream(1) fed
  // into video.srcObject, which needs no asset at all, but it is an unverified
  // technique and it would trade something known to work -- the video branch is
  // asserted against a real browser in tools/data-browser-smoke.js -- for about
  // five kilobytes.
  // ---------------------------------------------------------------------------
  const MEDIA = {mp4: "", webm: ""};

  if (typeof window !== "undefined" && window.document) autostart(window);

  return {create, attachDot, autostart, isIos, isMobile, MEDIA, STATES, LABELS, TIPS};
});
